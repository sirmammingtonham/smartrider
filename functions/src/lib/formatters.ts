  ...
  for (const trip of timetable.orderedTrips) {
    ...
    for (const stop of stops) {
      ...
      if (!lastStopTime || lastStopTime.trip_id !== trip.trip_id) {
        const stoptime = createEmptyStoptime(stop.stop_id, trip.trip_id);
        stop.trips.push(formatStopTime(stoptime, timetable, config));
      }
    }
  }
...
exports.formatTimetableLabel = (timetable: any) => {
  ...
  if (!exports.isNullOrEmpty(timetable.routes[0].route_short_name)) {
    timetableLabel += timetable.routes[0].route_short_name;
  } else if (!exports.isNullOrEmpty(timetable.routes[0].route_long_name)) {
  ...
  if (timetable.stops && timetable.stops.length > 0) {
    ...
    if (firstStop === lastStop) {
      if (!exports.isNullOrEmpty(timetable.routes[0].route_long_name)) {
        timetableLabel += ` - ${timetable.routes[0].route_long_name}`;
      }
