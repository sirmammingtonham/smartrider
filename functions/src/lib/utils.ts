...
const findCommonStopId = (trips: any, config: any) => {
  ...
  const commonStoptime = longestTripStoptimes.find((stoptime: any, idx: any) => {
    ...
    if (formatters.isNullOrEmpty(stoptime.arrival_time)) {
      return false;
    }
...
const getTimetableNotes = async (timetable: any, config: any) => {
  ...
  const notes = await gtfs.getTimetableNotes({ note_id: usedNoteReferences.map(noteReference => noteReference.note_id) });
  ...
  for (const frequency of frequencies) {
    ...
    for (let offset = startSeconds; offset < endSeconds; offset += frequency.headway_secs) {
      ...
      trips.push({
        ...
        stoptimes: formatters.updateStoptimesByOffset(newTrip, offset)
      });
    }
  }
...
const getAllStationStopIds = async (stopId: any) => {
  ...
  if (formatters.isNullOrEmpty(stop.parent_station)) {
    return [stopId];
  }
