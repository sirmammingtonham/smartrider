/*
Name: Akshath Jain Date: 3/18/2019 - 4/2/2020 Purpose: Defines the
sliding_up_panel widget Copyright: © 2020, Akshath Jain. All rights reserved.
Licensing: More information can be found here:
https://github.com/akshathjain/sliding_up_panel/blob/master/LICENSE
*/

// sliding up panel, modified so we can drag the header widget while maintaining
// a scrollview in the body (and to our linting rules)
import 'dart:math';
...
class SlidingUpPanel extends StatefulWidget {
  ...
  /// The Widget that slides into view. When the panel is collapsed and if
  /// [collapsed] is null, then top portion of this Widget will be displayed;
  /// otherwise, [collapsed] will be displayed overtop of this Widget. If
  /// [panel] and [panelBuilder] are both non-null, [panel] will be used.
  final Widget? panel;
  ...
  /// The Widget displayed overtop the [panel] when collapsed. This fades out as
  /// the panel is opened.
  ...
  /// Optional persistent widget that floats above the [panel] and attaches to
  /// the top of the [panel]. Content at the top of the panel will be covered by
  /// this widget. Add padding to the bottom of the `panel` to avoid coverage.
  final Widget? header;

  /// Optional persistent widget that floats above the [panel] and attaches to
  /// the bottom of the [panel]. Content at the bottom of the panel will be
  /// covered by this widget. Add padding to the bottom of the `panel` to avoid
  /// coverage.
  final Widget? footer;
  ...
  /// animating. A fast swipe on the panel will disregard this point and go
  /// directly to the open/close position. This value is represented as a
  /// percentage of the total animation distance ([maxHeight] - [minHeight]), so
  /// it must be between 0.0 and 1.0, exclusive.
  ...
  /// Set to false to not to render the sheet the [panel] sits upon. This means
  /// that only the [body], [collapsed], and the [panel] Widgets will be
  /// rendered. Set this to false if you want to achieve a floating effect or
  /// want more customization over how the sliding panel looks like.
  ...
  /// The opacity of the backdrop when the panel is fully open. This value can
  /// range from 0.0 to 1.0 where 0.0 is completely transparent and 1.0 is
  /// completely opaque.
  final double backdropOpacity;

  /// Flag that indicates whether or not tapping the backdrop closes the panel.
  /// Defaults to true.
  final bool backdropTapClosesPanel;

  /// If non-null, this callback is called as the panel slides around with the
  /// current position of the panel. The position is a double between 0.0 and
  /// 1.0 where 0.0 is fully collapsed and 1.0 is fully open.
  ...
  /// If non-null and true, the SlidingUpPanel exhibits a parallax effect as the
  /// panel slides up. Essentially, the body slides up as the panel slides up.
  final bool parallaxEnabled;

  /// Allows for specifying the extent of the parallax effect in terms of the
  /// percentage the panel has slid up/down. Recommended values are within 0.0
  /// and 1.0 where 0.0 is no parallax and 1.0 mimics a one-to-one scrolling
  /// effect. Defaults to a 10% parallax.
  final double parallaxOffset;

  /// Allows toggling of the draggability of the SlidingUpPanel. Set this to
  /// false to prevent the user from being able to drag the panel up and down.
  /// Defaults to true.
  final bool isDraggable;

  /// Either SlideDirection.UP or SlideDirection.DOWN. Indicates which way the
  /// panel should slide. Defaults to UP. If set to DOWN, the panel attaches
...
class _SlidingUpPanelState extends State<SlidingUpPanel>
                ...
                child: AnimatedBuilder(
                    ...
                    builder: (context, _) {
                      return Container(
                        ...
                        //set color to null so that touch events pass through to
                        //the body when the panel is closed, otherwise, if a
                        //color exists, then touch events won't go through
                        color: _ac.value == 0.0
                            ? null
                        ...
                        child: SizedBox(
                          ...
                          child: widget.collapsed == null
                              ...
                              : FadeTransition(
                                  ...
                                  // touch events go through to whatever is
                                  // underneath
                                  child: IgnorePointer(
  ...
  // returns a gesture detector if panel is used and a listener if panelBuilder
  // is used. this is because the listener is designed only for use with linking
  // the scrolling of panels and using it for panels that don't want to linked
    ...
    if (widget.panel != null) {
      return GestureDetector(
        onVerticalDragUpdate: (DragUpdateDetails dets) =>
            _onGestureSlide(dets.globalPosition.dy, dets.delta.dy),
  ...
  // handles the sliding gesture void _onGestureSlide(double dy) {
  void _onGestureSlide(double pos, double delta) {
    // only slide the panel if scrolling is not enabled
    if (!_scrollingEnabled) {
      if (widget.slideDirection == SlideDirection.up) {
        _ac.value -= delta / (widget.maxHeight - widget.minHeight);
      } else {
        _ac.value += delta / (widget.maxHeight - widget.minHeight);
      }
    }

    // determines if a tap has dragged on the header widget and enables the
    // widget to be dragged from it
    if (_isPanelOpen && _sc.hasClients && widget.header != null) {
      final box = _headerKey.currentContext?.findRenderObject() as RenderBox;
      final headerPos = box.localToGlobal(Offset.zero).dy;
      setState(() {
        if (headerPos < pos && pos < headerPos + box.size.height) {
  ...
  void _onGestureEnd(Velocity v) {
    ...
    // if scrolling is allowed and the panel is open, we don't want to close the
    // panel if they swipe up on the scrollable
  ...
  //get the current panel position returns the % offset from collapsed state as
...
class PanelController {
  ...
  /// Determine if the panelController is attached to an instance of the
  /// SlidingUpPanel (this property must return true before any other functions
  /// can be used)
  ...
  /// Shows the sliding panel in its collapsed state (i.e. "un-hide" the sliding
  /// panel)
  Future<void> show() {
    assert(isAttached, 'PanelController must be attached to a SlidingUpPanel');
  ...
  /// Animates the panel position to the value. The value must between 0.0 and
  /// 1.0 where 0.0 is fully collapsed and 1.0 is completely open. (optional)
  ...
  /// Animates the panel position to the snap point Requires that the
  /// SlidingUpPanel snapPoint property is not null (optional) duration
  ...
  /// Sets the panel position (without animation). The value must between 0.0
  /// and 1.0 where 0.0 is fully collapsed and 1.0 is completely open.
  ...
  /// Gets the current panel position. Returns the % offset from collapsed state
  /// to the open state as a decimal between 0.0 and 1.0 where 0.0 is fully
  /// collapsed and 1.0 is full open.
